 ...
 * getTierDisplayName('explorer') // returns "Free Tier" (explorer is the free tier)
...
export function getTierDisplayName(tier: string | null | undefined): string {
  const normalizedTier = (tier || 'explorer').toLowerCase();
 ...
 * getTierDisplayNameShort('explorer') // returns "Free" (explorer is the free tier)
...
export function isPaidTier(tier: string | null | undefined): boolean {
  const normalizedTier = (tier || 'explorer').toLowerCase();
  return normalizedTier !== 'free' && normalizedTier !== 'explorer';
}
...
export function getTierInfo(tier: string | null | undefined) {
  ...
  > = {
    free: {
      ...
      isPaid: false,
      description: 'Get started with 5 blueprints per month',
      maxGenerations: 5,
      maxSaved: 5,
    },
    explorer: {
 ...
 *
 * @example
 * getTierMaxGenerations('free') // returns 5
 * getTierMaxGenerations('explorer') // returns 5 (explorer is the free tier)
 ...
 *
 * @example
 * getTierMaxSaved('free') // returns 5
 * getTierMaxSaved('explorer') // returns 5 (explorer is the free tier)
 ...
 * Note: 'explorer' is treated as the free tier (same as 'free')
 */
const INDIVIDUAL_TIER_HIERARCHY = ['explorer', 'navigator', 'voyager'];
...
export function getAvailableUpgradePlans(currentTier: string | null | undefined): {
  ...
  const normalizedTier = (currentTier || 'explorer').toLowerCase();

  // Treat 'free' as 'explorer' since they're the same tier
  const effectiveTier = normalizedTier === 'free' ? 'explorer' : normalizedTier;

  // Check if user is on an individual tier
  const individualTierIndex = INDIVIDUAL_TIER_HIERARCHY.indexOf(effectiveTier);
  const isIndividualTier = individualTierIndex !== -1;

  // Check if user is on a team tier
  const teamTierIndex = TEAM_TIER_HIERARCHY.indexOf(effectiveTier);
  ...
  if (isIndividualTier) {
    ...
    // Unknown tier or null - show all plans (same as explorer/free tier)
    individualPlans = INDIVIDUAL_TIER_HIERARCHY.slice(1); // Skip 'explorer' itself
    teamPlans = [...TEAM_TIER_HIERARCHY];
  }
