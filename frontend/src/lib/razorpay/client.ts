...
export function getRazorpayClient(): Razorpay {
  ...
  if (typeof window !== 'undefined') {
    throw new Error(
      '⛔ SECURITY ERROR: Razorpay server client cannot be used in browser. ' +
        'This client contains secret keys and must only be used in server-side code (API routes, server components). ' +
        'For client-side checkout, use the Razorpay Checkout SDK loaded via script tag.'
    );
  }
...
/**
 * Lazy-initialized Razorpay client for server-side use
 *
 * @description This provides a singleton-like pattern while maintaining lazy initialization.
 * The client is only created when first accessed, preventing build-time errors.
 *
 * **SECURITY**: Includes runtime browser check to prevent accidental client-side usage
 *
 * @example Server-side API route
 * ```typescript
 * import { razorpayClient } from '@/lib/razorpay/client';
 ...
 * ```
 */
let _razorpayClientInstance: Razorpay | null = null;

export const razorpayClient = new Proxy({} as Razorpay, {
  get(target, prop) {
    // Lazy initialization on first property access
    if (!_razorpayClientInstance) {
      try {
        _razorpayClientInstance = getRazorpayClient();
      } catch (error) {
        // During build time or if config is missing, return undefined
        // This allows imports to succeed during build without executing
    ...
    return _razorpayClientInstance[prop as keyof Razorpay];
  },
});
