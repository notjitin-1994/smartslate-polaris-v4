 ...
 *
 * Triggered when a customer successfully authenticates a subscription payment.
 * This event occurs before subscription.activated for authentication-based payment methods.
 */
export const handleSubscriptionAuthenticated: EventHandler = async (
  event: ParsedWebhookEvent
): Promise<EventHandlerResult> => {
  const subscription = extractSubscriptionEntity(event);
  const supabase = await getSupabaseServerClient();
  ...
  try {
    ...
    // Update subscription record with authenticated status
    const updateData: SubscriptionUpdateData = {
    ...
    return {
      ...
      details: {
        ...
        metadata: {
          userId,
          message: 'Customer successfully authenticated subscription payment',
        },
      },
    };
  } catch (error) {
    return {
      success: false,
      processed: false,
      error: `Failed to process subscription authentication: ${error instanceof Error ? error.message : 'Unknown error'}`,
      retryable: true,
    };
  }
};
