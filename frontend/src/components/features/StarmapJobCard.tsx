...
/**
* Map a job status to a human-readable label and corresponding CSS utility classes.
* @example
* getStatusTone('completed')
* { text: 'Completed', classes: 'text-green-300 bg-green-400/15 border-green-400/30' }
* @param {{string}} status - The Starmap job status (e.g. 'completed', 'processing', 'queued', 'failed', 'cancelled').
* @returns {{{ text: string; classes: string }}} An object containing the display text and CSS classes for the provided status.
**/
...
/**
* Compute a job's progress percentage using reported async progress when available, otherwise deriving from completed stages.
* @example
* useProgress({ report_job_progress: 42 })
* 42
* @param {{StarmapJob}} {{job}} - Starmap job object containing progress value and stage completion flags.
* @returns {{number}} Returns a clamped integer percentage (0–100) representing the job's progress.
**/
...
export function StarmapJobCard({ job, onView, onResume, onDelete, deleting }: StarmapJobCardProps) {
  ...
  /**
  * Normalize a metric label/detail into a structured metric object (title, baseline, target, timeframe).
  * @example
  * normalizeMetric({ label: 'Revenue {"metric":"revenue","baseline":"$10k","target":"$15k","timeframe":"Q4"}' })
  * { title: 'revenue', baseline: '$10k', target: '$15k', timeframe: 'Q4' }
  * @param {{ label: string; detail?: string }} item - Input object with a label and optional detail containing metric information.
  * @returns {{ title: string; baseline?: string; target?: string; timeframe?: string }} Normalized metric object with parsed title and optional baseline, target, and timeframe.
  **/
  ...
  /**
  * Parse a string representing a date (ISO-like or partial) and return its timestamp in milliseconds or null.
  * @example
  * parseIsoOrDateish('2023-05-10')
  * 1683676800000
  * @param {{string|undefined}} {{input}} - Input date string in ISO-like formats (e.g. 'YYYY-MM-DD', 'YYYY-MM') or any format accepted by Date.parse; optional.
  * @returns {{number|null}} Unix timestamp in milliseconds, or null if parsing failed.
  **/
