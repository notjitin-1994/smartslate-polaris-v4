...
/**
* Render a modal that previews and lets the user download export files in multiple formats (PDF, Markdown, JSON, DOCX).
* @example
* ExportPreviewModal({isOpen: true, onClose: () => {}, exportResults: new Map(), metadata: { title: 'Report', exportedAt: '2025-01-01' }, onDownload: (format) => {}, onDownloadAll: () => {} })
* <JSX.Element /> or null
* @param {{boolean}} {{isOpen}} - Whether the export preview modal is open.
* @param {{() => void}} {{onClose}} - Callback invoked to close the modal.
* @param {{Map<string, { success: boolean, data?: Blob | Response, error?: string }>|Map<ExportFormat, any>}} {{exportResults}} - Map of export format keys to their result objects (success flag, optional data blob/response, optional error).
* @param {{title: string, exportedAt: string}} {{metadata}} - Metadata about the export (title and exportedAt timestamp) displayed in the modal header.
* @param {{(format: ExportFormat) => void}} {{onDownload}} - Callback to download a single format; receives the selected format.
* @param {{() => void}} {{onDownloadAll}} - Callback to download all available export formats.
* @returns {{JSX.Element | null}} Rendered modal JSX element when open, otherwise null.
**/
