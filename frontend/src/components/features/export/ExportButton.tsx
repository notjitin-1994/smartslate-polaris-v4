...
/**
* Render export controls for a blueprint and handle exporting, previewing, and downloading in multiple formats.
* @example
* ExportButton({ blueprint: myBlueprint, dashboardData: myDashboardData, metadata: myMetadata, className: 'mx-2', showPreview: true })
* <JSX.Element />
* @param {Object} blueprint - Blueprint object to export (used to derive filenames like blueprint.title).
* @param {Object} dashboardData - Dashboard data included with the export.
* @param {Object|null} metadata - Optional metadata to include in the exported output.
* @param {string} className - Optional additional CSS class names applied to the button container.
* @param {boolean} showPreview - Whether to show the export preview modal instead of automatically downloading.
* @returns {JSX.Element} Rendered export button group and optional preview modal.
**/
export const ExportButton: React.FC<ExportButtonProps> = ({
  ...
  /**
  * Trigger an export of the current blueprint in the given format, showing a preview modal or downloading the file while managing loading and error state.
  * @example
  * sync('pdf')
  * Promise<void>
  * @param {{ExportFormat}} {{format}} - Export format to use (e.g. 'pdf', 'xlsx').
  * @returns {{Promise<void>}} Returns a promise that resolves when the export process completes.
  **/
  ...
  /**
  * Exports the current blueprint into multiple formats (pdf, markdown, json, docx), collects results, updates export metadata and UI state, and shows the export modal.
  * @example
  * sync()
  * // Starts exporting in parallel for pdf, markdown, json, and docx, then opens the export results modal or shows an alert on failure.
  * @returns {Promise<void>} Resolves when all export attempts complete and the component's exporting state and modal visibility have been updated.
  */
