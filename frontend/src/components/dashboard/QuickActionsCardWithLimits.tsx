...
import { Plus, FileText, Rocket, BookOpen, Compass } from 'lucide-react';
...
const quickActions = [
  ...
  },
  {
    icon: BookOpen,
    title: 'Learn More',
    description: 'Features, best practices, and workflows',
    href: '/features',
    color: 'from-indigo-600 to-indigo-700',
    bgColor: 'bg-indigo-600/10',
    borderColor: 'border-indigo-600/20',
  },
  {
    ...
    description: 'Explore Solara Suite of Applications',
...
export function QuickActionsCardWithLimits() {
  ...
  const handleActionClick = async (action: (typeof quickActions)[0]) => {
    ...
    // Check if user is on mobile/tablet and trying to create a blueprint
    if (action.requiresLimitCheck && isNonDesktop && isMounted) {
      console.log('📱 Non-desktop device detected! Showing desktop-only modal');
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {quickActions.map((action, index) => {
            ...
            const content = (
              <div
                className={cn(
                  'group h-full w-full rounded-xl border p-4 transition-all duration-200',
                  action.borderColor,
                  action.bgColor,
            ...
            return (
              ...
              >
                {action.requiresLimitCheck ? (
                  <div onClick={() => handleActionClick(action)} className="h-full w-full">
                    {content}
                  </div>
                ) : action.isExternal ? (
                  <a
                  ...
                  <Link href={action.href} className="block h-full w-full">
                    {content}
                  </Link>
                )}
              </motion.div>
