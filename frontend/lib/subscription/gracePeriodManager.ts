...
function getDefaultLimits(tier: string): { creationLimit: number; savingLimit: number } {
  // IMPORTANT: These limits must match the database tier limits
  // See: supabase/migrations/20251102120000_fix_tier_limits_consistency.sql
  const limits: Record<string, { creationLimit: number; savingLimit: number }> = {
    free: { creationLimit: 2, savingLimit: 2 },
    explorer: { creationLimit: 5, savingLimit: 5 },
    navigator: { creationLimit: 25, savingLimit: 25 },
    voyager: { creationLimit: 50, savingLimit: 50 },
    crew: { creationLimit: 10, savingLimit: 10 },
    fleet: { creationLimit: 30, savingLimit: 30 },
    armada: { creationLimit: 60, savingLimit: 60 },
    developer: { creationLimit: -1, savingLimit: -1 }, // Unlimited
  };
