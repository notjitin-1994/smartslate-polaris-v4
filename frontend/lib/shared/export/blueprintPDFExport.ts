...
function generateCoverPage(title: string, date: string): string {
  return `
    <div style="padding: 80px 60px; text-align: center; page-break-after: always; min-height: 1000px; display: flex; flex-direction: column; justify-content: center; background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%);">
      ...
      <h1 style="font-size: 42px; font-weight: 800; color: #0f172a; margin: 40px 0; line-height: 1.2; font-family: system-ui, -apple-system, sans-serif;">${escapeHtml(title)}</h1>
      <p style="color: #64748b; font-size: 18px; margin: 30px 0 60px 0;">Created on ${escapeHtml(date)}</p>
...
function generateMetricCard(label: string, value: string, color: string): string {
  return `
    <div style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border: 2px solid #e2e8f0; border-radius: 16px; padding: 28px 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
      <div style="width: 48px; height: 48px; background: ${escapeHtml(color)}20; border-radius: 12px; margin-bottom: 16px; display: flex; align-items: center; justify-content: center;">
        <div style="width: 24px; height: 24px; background: ${escapeHtml(color)}; border-radius: 50%;"></div>
      </div>
      <p style="color: #64748b; font-size: 13px; margin: 0 0 8px 0; font-weight: 500;">${escapeHtml(label)}</p>
      <p style="color: #0f172a; font-size: 36px; font-weight: 800; margin: 0; line-height: 1;">${escapeHtml(value)}</p>
    </div>
  `;
}
      ...
      <div style="display: grid; grid-template-columns: repeat(1, 1fr); gap: 14px;">
        ${objectives
          ...
          <div style="background: linear-gradient(135deg, #f0f9ff 0%, #f8fafc 100%); border-left: 4px solid #7bc5c7; border-radius: 8px; padding: 16px 20px; display: flex; align-items: start; gap: 14px;">
            ...
            <span style="color: #334155; font-size: 14px; line-height: 1.6; flex: 1;">${escapeHtml(obj)}</span>
          </div>
        `
          )
          .join('')}
            ...
            return `
              <div style="background: linear-gradient(135deg, #faf5ff 0%, #f8fafc 100%); border: 2px solid #e9d5ff; border-radius: 14px; padding: 20px 24px;">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 14px;">
                  <h4 style="color: #0f172a; font-size: 16px; font-weight: 600; margin: 0; flex: 1;">${escapeHtml(module.title)}</h4>
                  <span style="background: linear-gradient(135deg, #7bc5c7 0%, #4F46E5 100%); color: #ffffff; padding: 6px 14px; border-radius: 20px; font-size: 11px; font-weight: 700; white-space: nowrap; margin-left: 12px;">Module ${escapeHtml(index + 1)}</span>
                </div>
                <div style="display: flex; gap: 20px; color: #64748b; font-size: 13px; font-weight: 500;">
                  <span>⏱ ${escapeHtml(duration)}h</span>
                  <span>📚 ${escapeHtml(topicCount)} topics</span>
                  <span>🎯 ${escapeHtml(activityCount)} activities</span>
                </div>
              </div>
            `;
          })
  ...
  const markdownToHtml = (md: string) => {
    return md
      .replace(
        /^### (.*$)/gim,
        (_, content) =>
          `<h3 style="color: #334155; font-size: 20px; font-weight: 600; margin: 28px 0 14px 0; line-height: 1.4;">${escapeHtml(content)}</h3>`
      )
      .replace(
        /^## (.*$)/gim,
        (_, content) =>
          `<h2 style="color: #0f172a; font-size: 26px; font-weight: 700; margin: 36px 0 18px 0; padding-bottom: 10px; border-bottom: 2px solid #e2e8f0; line-height: 1.3;">${escapeHtml(content)}</h2>`
      )
      .replace(
        /^# (.*$)/gim,
        (_, content) =>
          `<h1 style="color: #0f172a; font-size: 32px; font-weight: 800; margin: 40px 0 24px 0; padding-bottom: 14px; border-bottom: 3px solid #7bc5c7; line-height: 1.2;">${escapeHtml(content)}</h1>`
      )
      .replace(
        /\*\*(.*?)\*\*/gim,
        (_, content) =>
          `<strong style="color: #0f172a; font-weight: 700;">${escapeHtml(content)}</strong>`
      )
      .replace(
        /\*(.*?)\*/gim,
        (_, content) =>
          `<em style="color: #4F46E5; font-style: italic;">${escapeHtml(content)}</em>`
      )
      .replace(
        /^\* (.+$)/gim,
        (_, content) =>
          `<li style="color: #475569; margin-bottom: 10px; line-height: 1.7; list-style-position: outside; margin-left: 20px;">${escapeHtml(content)}</li>`
      )
      .replace(
        /^\d+\. (.+$)/gim,
        (_, content) =>
  ...
  return `
    <div style="padding: 50px 40px; background: #ffffff;">
      ...
      <div style="margin-top: 80px; padding-top: 30px; border-top: 2px solid #e2e8f0; text-align: center;">
        <p style="color: #94a3b8; font-size: 13px; font-weight: 500;">Generated with SmartSlate AI • ${escapeHtml(date)}</p>
      </div>
    </div>
  `;
}
