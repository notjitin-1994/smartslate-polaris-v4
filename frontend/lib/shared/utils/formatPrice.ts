 ...
 */
export const formatPrice = (usdPrice: number, options: PriceFormatOptions): string => {
  const { currency, exchangeRate, showDecimals = false, showSymbol = true } = options;

  if (currency === 'USD') {
    const formatted = showDecimals ? usdPrice.toFixed(2) : Math.round(usdPrice).toString();
