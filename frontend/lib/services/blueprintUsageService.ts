...
export class BlueprintUsageService {
   ...
   * DEPRECATED: Use incrementCreationCountV2 for fail-closed semantics
   */
  static async incrementCreationCount(supabase: SupabaseClient, userId: string): Promise<boolean> {
    ...
    if (error) {
      console.error('Error incrementing blueprint creation count:', {
        error,
      ...
      throw new Error(
        `Failed to increment blueprint creation count: ${error.message || 'Unknown error'}`
      );
    }
  ...
  /**
   * Increment blueprint creation count with fail-closed semantics (CVE-001 fix)
   * Returns detailed result including success status, reason, and new count
   * FAIL-CLOSED: Denies operation on any error condition
   */
  static async incrementCreationCountV2(
    supabase: SupabaseClient,
    userId: string
  ): Promise<{ success: boolean; reason: string; newCount: number }> {
    try {
      const { data, error } = await supabase.rpc('increment_blueprint_creation_count_v2', {
        p_user_id: userId,
      });

      // FAIL-CLOSED: Treat database errors as denial
      if (error) {
        console.error('[SECURITY] Failed to increment creation count - DENYING', {
          userId,
          error: error.message,
          code: error.code,
        });
      ...
      // FAIL-CLOSED: Treat missing data as denial
      if (!data || data.length === 0) {
        console.error('[SECURITY] No data returned from increment - DENYING', { userId });
        return {
          success: false,
          reason: 'System error - unable to verify limits',
          newCount: 0,
      ...
      // Log for audit trail
      if (!result.success) {
        console.warn('[LIMITS] Blueprint creation denied', {
          userId,
          reason: result.reason,
          currentCount: result.new_count,
        });
      } else {
        console.log('[LIMITS] Blueprint creation allowed', {
          userId,
      ...
      // FAIL-CLOSED: All exceptions result in denial
      console.error('[SECURITY] Exception in incrementCreationCountV2 - DENYING', {
        userId,
   ...
   * DEPRECATED: Use incrementSavingCountV2 for fail-closed semantics
   */
  ...
  /**
   * Increment blueprint saving count with fail-closed semantics (CVE-001 fix)
   * Returns detailed result including success status, reason, and new count
   * FAIL-CLOSED: Denies operation on any error condition
   */
  static async incrementSavingCountV2(
    supabase: SupabaseClient,
    userId: string
  ): Promise<{ success: boolean; reason: string; newCount: number }> {
    try {
      const { data, error } = await supabase.rpc('increment_blueprint_saving_count_v2', {
        p_user_id: userId,
      });

      // FAIL-CLOSED: Treat database errors as denial
      if (error) {
        console.error('[SECURITY] Failed to increment saving count - DENYING', {
          userId,
          error: error.message,
          code: error.code,
        });
      ...
      // Log for audit trail
      if (!result.success) {
        console.warn('[LIMITS] Blueprint saving denied', {
          userId,
          reason: result.reason,
          currentCount: result.new_count,
        });
      } else {
        console.log('[LIMITS] Blueprint saving allowed', {
          userId,
          newCount: result.new_count,
        });
      }
      ...
      // FAIL-CLOSED: All exceptions result in denial
      console.error('[SECURITY] Exception in incrementSavingCountV2 - DENYING', {
        userId,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      return {
