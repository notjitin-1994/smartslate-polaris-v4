...
import { feedbackSubmissionSchema } from '@/lib/schemas/feedbackSchemas';
  ...
  try {
    ...
    const { data: feedback, error: dbError } = await supabase
      ...
      .insert({
        ...
        page_url: validatedData.pageUrl || request.headers.get('referer') || null,
        status: 'new',
      })
      .select('id')
      .single();
    ...
    fetch(`${request.nextUrl.origin}/api/feedback/send-email`, {
      method: 'POST',
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          console.error('[FEEDBACK] Email API returned error:', res.status, errorData);
        } else {
          const successData = await res.json();
          console.log('[FEEDBACK] Email sent successfully:', successData);
        }
      })
      .catch((error) => {
        console.error('[FEEDBACK] Failed to call email API:', error);
    ...
    return NextResponse.json(
      {
        ...
        data: {
          id: feedback.id,
          message: "Thank you for your feedback! We'll review it within 48 hours.",
        },
      },
      { status: 201 }
    );
