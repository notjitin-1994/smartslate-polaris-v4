...
/**
* Renders a labeled currency input with formatting, currency symbol, validation and helper text.
* @example
* CurrencyInput({ label: 'Amount', value: 1000, onChange: (v) => console.log(v) })
* React.JSX.Element
* @param {string} label - Field label text.
* @param {number} value - Numeric value to display (interpreted as whole currency units).
* @param {(value: number) => void} onChange - Callback invoked with the new numeric value when the input changes.
* @param {string} currency - Currency code used to display the currency symbol (default: 'USD').
* @param {string} placeholder - Placeholder text shown when the value is empty (default: '0').
* @param {string|undefined} error - Optional error message to display below the input.
* @param {string|undefined} helpText - Optional helper text shown above the input.
* @param {boolean} required - Whether the input is required and should show an asterisk (default: false).
* @param {boolean} disabled - Whether the input is disabled (default: false).
* @param {boolean} allowApproximate - Whether to show a note that approximate amounts are acceptable (default: false).
* @param {string|undefined} className - Optional additional CSS class names to apply to the component container.
* @returns {React.JSX.Element} Rendered currency input component.
**/
