...
/**
 * Renders a labeled input or textarea with optional help text and error display for questionnaire-style forms.
 * @example
 * QuestionnaireInput({ label: 'Name', value: 'Alice', onChange: v => {}, placeholder: 'Enter name', required: true })
 * <JSX.Element />
 * @param {string} label - Visible label text associated with the input.
 * @param {string} value - Current input value.
 * @param {(value: string) => void} onChange - Callback invoked with the new value when the input changes.
 * @param {string} placeholder - Placeholder text shown when the input is empty.
 * @param {string | undefined} error - Optional error message to display below the input; toggles error styling when present.
 * @param {string | undefined} helpText - Optional helper text shown above the input.
 * @param {boolean} required - If true, marks the field as required and shows a required indicator.
 * @param {boolean} multiline - If true, renders a textarea (multiline) instead of a single-line input.
 * @param {number} rows - Number of rows to use for the textarea when multiline is true.
 * @returns {React.JSX.Element} A JSX element containing the labeled input/textarea, help text, and error UI.
 */
