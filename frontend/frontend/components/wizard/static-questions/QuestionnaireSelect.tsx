...
/**
* Renders a styled select input for questionnaire steps with label, helper text, options, error display, and required indicator.
* @example
* QuestionnaireSelect({ label: 'Choose', value: '', onChange: v => console.log(v), options: [{ label: 'One', value: '1' }] })
* <QuestionnaireSelect ... />
* @param {string} label - Visible label text shown above the select.
* @param {string} value - Currently selected option value.
* @param {(value: string) => void} onChange - Callback invoked when the selection changes; receives the new value.
* @param {Array<{label: string, value: string, disabled?: boolean}>} options - Array of option objects to render inside the select.
* @param {string} placeholder - Placeholder text shown as a disabled option when no value is selected.
* @param {string | undefined} error - Optional error message; when provided, an error message is displayed below the select.
* @param {string | undefined} helpText - Optional helper text displayed beneath the label.
* @param {boolean} required - Whether the field is required; if true an asterisk is shown next to the label.
* @returns {React.JSX.Element} Rendered React JSX element for the questionnaire select control.
**/
