-- ==============================================================
-- FINAL SAFE MIGRATION SCRIPT FOR COST TRACKING
-- ==============================================================
-- This script handles all existing objects gracefully
...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'api_usage_logs') THEN
    ...
    CREATE INDEX idx_api_usage_blueprint ON public.api_usage_logs(blueprint_id);
...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'api_model_pricing') THEN
    ...
    CREATE INDEX idx_model_pricing_active ON public.api_model_pricing(provider, model_id, is_active)
...
BEGIN
  IF NOT EXISTS (
    ...
    AND indexname = 'unique_active_model_pricing'
  ) THEN
...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_cost_summaries') THEN
    ...
    CREATE INDEX idx_cost_summaries_user_period ON public.user_cost_summaries(user_id, period_type, period_date DESC);
    CREATE INDEX idx_cost_summaries_period_date ON public.user_cost_summaries(period_date DESC);
...
-- Function to update cost summaries
...
BEGIN
  ...
  RAISE NOTICE '  - update_cost_summaries';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Next step: Run manual_backfill_corrected.sql to populate historical data';
  RAISE NOTICE '';
END $$;
